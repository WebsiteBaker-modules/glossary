...
include('info.php');
// Get ID
if (isset($_POST['term_id']) AND is_numeric($_POST['term_id'])) {
...
// Get post details
$query_details = $database->query("SELECT * FROM `".TABLE_PREFIX."mod_".$module_directory."` WHERE `term_id`=".$term_id);
if($query_details->numRows() > 0) {
	$get_details = $query_details->fetchRow( MYSQL_ASSOC );
} else {
	$admin->print_error(
		$TEXT['NOT_FOUND'],
		ADMIN_URL.'/pages/modify.php?page_id='.$page_id
...
$database->query("DELETE FROM `".TABLE_PREFIX."mod_".$module_directory."` WHERE `term_id`=".$term_id);
