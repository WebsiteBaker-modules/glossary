...
class c_glossary_query
{
	private $guid = "A025BCED-30F5-4CF2-8086-B29409D71339";
	...
	public function validate_value(&$aValue, &$options) {
		$ret_val = true;
		switch(strtolower($options['type'])) {
			case 'int+':
			case 'i+':
				$ret_val = (($aValue > $options['min']) && ($aValue <= $options['max']));
				break;
		}
		return $ret_val;
	}
